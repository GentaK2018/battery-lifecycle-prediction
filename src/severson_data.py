...
def get_max_life_time(data_dict):
    ...
    for bat in data_dict.keys():
        max_lifetime = np.maximum(max_lifetime,
                                  data_dict[bat]["cycle_life"].flatten()[0])
...
def assemble_dataset(x, y, augment, seq_len=50, use_cycle_counter=True):
    ...
    ys = []
    add_data_s = []
    for i in range(len(x)):  # xのサンプルサイズだけ繰り返し
        # サイクル寿命が400で、seq_lengthが100ならば
        # 400-100-3=297までjを繰り返す。
        for j in range(y[i] - seq_len - 3):  

            xs.append(x[i, j:j + seq_len]) # jから数えてseq_len分のデータを選ぶ

            # 1個前の値に対する割合（≒1サイクルでどのくらいへたったか？）
